...
ESTADOS_API_FOOTBALL_ES = {
    ...
    "2H": "Segunda parte",
    "ET": "Prórroga",
    "BT": "Descanso de prórroga",
    "P": "Penaltis",
    "SUSP": "Suspendido",
    "INT": "Interrumpido",
    "FT": "Finalizado",
    "AET": "Finalizado tras prórroga",
...
def _nombre_seleccion_es(nombre):
    return NOMBRES_SELECCIONES_ES.get(nombre, nombre)


def _estado_api_football_es(estado):
    return ESTADOS_API_FOOTBALL_ES.get(estado, estado)


def _puntuacion_nations(item):
    teams = item.get("teams") or {}
    local = (teams.get("home") or {}).get("name", "")
    visitante = (teams.get("away") or {}).get("name", "")

    if local == "Spain" or visitante == "Spain":
        return 100000

    puntos = PRIORIDAD_NATIONS.get(local, 0) + PRIORIDAD_NATIONS.get(visitante, 0)

    if local in PRIORIDAD_NATIONS and visitante in PRIORIDAD_NATIONS:
        puntos += 500

    return puntos


def _seleccionar_destacados_nations(partidos, ahora):
    por_dia = {}
    ...
    for item in partidos:
        ...
        if not (timedelta(0) < (inicio - ahora) <= timedelta(hours=PUBLICAR_HORAS_ANTES)):
            continue

        puntuacion = _puntuacion_nations(item)
        if puntuacion <= 0:
            continue
        ...
        if actual is None or puntuacion > actual[0]:
            por_dia[dia] = (puntuacion, item)

    seleccionados = set()
    for _, item in por_dia.values():
        fid = (item.get("fixture") or {}).get("id")
        if fid is not None:
            seleccionados.add(int(fid))
...
async def _procesar_partido_api_football(context, datos, item, permitir_publicar=True):
    ...
    estado = (fixture.get("status") or {}).get("short", "")
    local_original = (teams.get("home") or {}).get("name", "Local")
    visitante_original = (teams.get("away") or {}).get("name", "Visitante")
    local = _nombre_seleccion_es(local_original)
    visitante = _nombre_seleccion_es(visitante_original)
...
async def revisar_porras(context):
    ...
    try:
        ...
        seleccionados_nations = _seleccionar_destacados_nations(
            partidos_nations,
            ahora,
        )
        ...
        for item in partidos_nations:
            ...
            permitir_publicar = (
                ...
                and int(fixture_id) in seleccionados_nations
            )
            ...
            if await _procesar_partido_api_football(
                ...
                datos,
                item,
                permitir_publicar=permitir_publicar,
            ):
                cambiado = True
    except Exception as e:
    ...
    if not key_nations:
        ...
        try:
            ...
            if not partidos_nations:
                ...
                seleccionados = _seleccionar_destacados_nations(
                    partidos_nations,
                    ahora,
                )
                lineas.append("✅ UEFA Nations League:")

                for item in partidos_nations[:12]:
                    fixture = item.get("fixture") or {}
                    teams = item.get("teams") or {}
                    ...
                    local = _nombre_seleccion_es(
                        (teams.get("home") or {}).get("name", "Local")
                    )
                    visitante = _nombre_seleccion_es(
                        (teams.get("away") or {}).get("name", "Visitante")
                    )
                    estado_raw = (fixture.get("status") or {}).get("short", "?")
                    estado = _estado_api_football_es(estado_raw)
                    fixture_id = fixture.get("id")

                    estrella = (
                        "⭐ PORRA ELEGIDA · "
                        if fixture_id is not None and int(fixture_id) in seleccionados
                        else "• "
                    )

                    lineas.append(
                        f"{estrella}{local} - {visitante} · "
                        f"{dt:%d/%m %H:%M} · {estado}"
                    )

                if not seleccionados:
                    lineas.append(
                        "ℹ️ No hay partido prioritario dentro de las próximas 36 h."
                    )
        except Exception as e:
