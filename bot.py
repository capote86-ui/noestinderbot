...
async def responder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mensaje.startswith("/") or mensaje.startswith("!"):
        ...
        if comando == "start" and update.effective_chat.type == "private":
            await inicio_privado_secretos(update, context)
            return
    ...
    if comando == "secretos":
        admins = await context.bot.get_chat_administrators(chat_id)
        admin_ids = [admin.user.id for admin in admins]
        await iniciar_secretos(update, context, admin_ids)
        return

    if comando == "cancelarsecretos":
        admins = await context.bot.get_chat_administrators(chat_id)
        admin_ids = [admin.user.id for admin in admins]
        await cancelar_secretos(update, context, admin_ids)
        return

    if comando == "rankingsecretos":
