...
preguntas_trivial = [
    ...
    },
    {
        ...
        "opciones": ["Guitarra", "Piano", "Violín", "Trompeta"],
        "correcta": "B"
    },
    {
        "pregunta": "¿Cuál es el metal líquido a temperatura ambiente?",
        "opciones": ["Oro", "Hierro", "Mercurio", "Plata"],
        "correcta": "C"
    },
    {
    ...
    },
    {
        ...
        "opciones": ["Fútbol", "Tenis", "Golf", "Rugby"],
        "correcta": "B"
    },
    {
        "pregunta": "¿Cuántos minutos tiene una hora?",
        "opciones": ["50", "60", "70", "90"],
        "correcta": "B"
    }
]


async def enviar_pregunta_trivial(context, chat_id):
...
async def cerrar_pregunta_trivial(context, chat_id):
    partida = trivials_activos[chat_id]
    indice = partida["indice"]
    pregunta = partida["preguntas"][indice]
    ...
    texto = (
        ...
        f"{correcta}) {pregunta['opciones']['ABCD'.index(correcta)]}\n\n"
    )
...
async def finalizar_trivial(context, chat_id):
    partida = trivials_activos[chat_id]
...
async def botones_trivial(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if accion == "trivia_start":
        ...
        if query.from_user.id != RAQUEL_ID:
            await query.answer("Solo Raquel puede empezar la partida.", show_alert=True)
            return
...
async def responder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id != RAQUEL_ID:
        ...
        if chat_id in trivials_activos:
            await update.message.reply_text(
                "Ya hay un trivial activo en este grupo."
            )
            return
        ...
        await update.message.reply_text(
            ...
            "Cuando estéis listos, Raquel pulsa ▶️ Empezar.",
            reply_markup=teclado
        )
        return
    ...
    if mensaje.startswith("!rankingtrivial") or mensaje.startswith("/rankingtrivial"):
        ...
        for i, (user_id, puntos) in enumerate(ranking, start=1):
            nombre = nombres_usuarios.get(user_id, "Alguien misterioso")
            texto += f"{i}. {nombre} — {puntos} puntos\n"
